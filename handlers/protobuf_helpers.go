...
func createVehicleProto(r *http.Request, w http.ResponseWriter) []byte {
    ...
    if err != nil {
        log.Println("Unable to read the request body.  %v", err)
    }

    err = prot.Unmarshal(body, vehicle)
    if err != nil {
	log.Println("Deserialization error  %v", err)
    }
    ...
    out, err := prot.Marshal(res)
    if err != nil {
        log.Println("Serialization error  %v", err)
    }
    return  out
}
/*
